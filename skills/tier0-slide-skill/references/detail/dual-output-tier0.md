...
3. 含 PPTX 的富媒体长稿：页数 1:1；HTML 保真；PPTX 可编辑字 + 嵌入图；视频可封面帧降级但页必须保留。**禁止**只写页标题的空壳幻灯；抽查每页应有正文和/或图片 shape。
4. 不要用 English dual demo 教「怎样排版」。
5. 富媒体长稿若无法塞进 dual `kind`：允许「template HTML + 脚本重建 PPTX」，但 PPTX 仍须内容完整（标题、要点、嵌入证据图），验收同下表。
