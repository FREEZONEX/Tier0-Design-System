...
23 个登记版式（S01–S23）+ Tier0 官网 VI 皮肤。

> ⚠️ 本 skill **只** 使用 `assets/template-tier0.html`。P23/P24 实验页在模板里仍用 `img-split` / `img-grid` 等历史 CSS 类名（见 HTML 源码），文档中称「图文分栏 / 证据网格」。
> 完整 VI：`references/tier0-vi-style-guide.md` · 组件：`references/components-tier0.md`

---

## Tier0 locked mode（必须先读）

生成正文页时不要把 Tier0 当成“自由组合的风格包”。默认只能使用 `references/tier0-layout-lock.md` 登记的 `S01-S23`。每个 slide 都必须在 `<section>` 上写 `data-layout="Sxx"`。

**关键约束**:

- 先读 `tier0-layout-lock.md`，再读本文件的 Sxx 骨架。
- 封面用 `TIER0-COVER-EDITORIAL`（白底左文 + 右侧亮绿动态 ASCII 点阵），封底用 `TIER0-CLOSING-SPLIT`。**禁止**把右侧点阵扩成满屏 accent ASCII 封面。
- 顶部标题默认左对齐；`chrome-min` 已提供页级定位，正文页不默认加 `■ SECTION LABEL`。需要时把它作为分点、流程或图表的本地标签（Plex Mono）。
- 大标题用 `.headline` 或 `h-xl`，weight **400**；关键词 `[Keyword]` weight **500**。
- 卡片用 `.tier0-card`，不用 glossy SaaS 圆角卡。
- 图标用 **IBM Carbon**，不用 Lucide。
- 地点/路线页：`S08 + Tier0 Map Component`（见 `tier0-map-component.md`）。
- 生成后运行 `node scripts/validate-tier0-deck.mjs index.html`。

---

## 设计语言基线 · Tier0 融合

**配色**（固定 Tier0 Green Signal，见 `themes-tier0.md`）
- `--paper` 纸白底 #ffffff(主背景)
- `--ink` 黑墨字 #0a0a0a(主文字 / Ink 反转块)
- `--accent` 固定 `#B2ED1D`（见 themes-tier0.md）
- `--text-primary / secondary / helper` 三级文字灰阶
- `--border-subtle` 1px 发丝细线 #e0e0e0

**排版 · Tier0 覆盖 Tier0 默认值**
- 字体:`IBM Plex Sans` + `IBM Plex Mono`（不是 Inter/Helvetica）
- 字重:**400 (Regular) 大标题** / **500 (Medium) 括号关键词与 step title** / **400 正文**
- ~~200 ExtraLight 大字~~ → Tier0 禁用；kpi-hero 也用 400
- Section label: `■` + Plex Mono 500 uppercase
- 中文大标题分档规则仍适用（见下文表格）
...
**中文深墨章节标题（S10）行距**
`ink-section-divider__title` 默认 `line-height:1.14`。两行汉字禁止用英文单行 hero 的 `0.9`，否则会贴死。详见 `composition-craft-tier0.md`。

**语言单一**
中文页不要在正文下再挂英文复述句；英文页同理。专有名词（MQTT、UNS）可保留。见 `composition-craft-tier0.md`。

**演示最小字号与字重阶梯**
Tier0不是网页说明页,投屏时不能出现 10-12px 的注释字。默认下限:
...
**字号与字重阶梯(Tier0核心)** — "越大越细,越小越粗"不是感性描述:
...
- 封面/accent 反白大标题内强调字用 `italic + weight 300`,不要用 accent 色(蓝压蓝看不见)
...
**2. 本地 label 必须服务于一个内容块，不要压成左右**
`.section-label` / `.t-meta` 默认属于某个分点、流程节点、图表或证据块；普通正文页的 page-level label 会占掉标题区，不要重复添加。真正的章节分割页才使用「label 在大标题上方」的上下结构。
...
| Accent 绿填充 | `.card-accent` | 唯一焦点 | 一组中突出一项 |
...
- 如果信息图、流程图、UI 情景图是按 S15 槽位重新生成的,必须让所有图片使用同一 `object-fit:cover` 比例;不要再加 `.fit-contain`,否则会变成小图漂在白框里
- Tier0图片优先比例:S22 顶部横幅 `21:9`;S15 多图格统一 `21:9` 或统一 `16:10`
- 生成 2-4 张配图时,必须先绑定原始版式槽位:单张大图 = S22;多图 = S15 图片证据网格;不要使用未登记的 P23/P24
...
**版式多样性硬规则**
Tier0 主题有 22 个登记版式,生成时要主动展示版式系统,不要把所有内容都做成 `head + grid-reveal + card`:

- 7-8 页 deck 至少使用 **6 个不同 S 编号版式**
- 不允许连续 3 页使用同一种主体结构(如三页连续 S19 / 普通卡片)
- 如果是"测试模板"或"我想看看效果",必须覆盖:封面、收尾、至少 1 个对比/时间线(S08/S11/S02)、至少 1 个结构图(S14/S17)、至少 1 个图片版式(S22 或 S15 图片格)
- 图片页不等于新发明一页。单图用 S22,2-4 图用 S15 的原始证据网格骨架改造
...
不要只看 HTML/CSS。Tier0 模板的还原度要同时从**浏览器视觉**和**代码结构**判断:

1. 同时打开三份页面:原始参考 PPT、当前 `template-tier0.html` 或生成页、正在修改的测试 PPT。原始参考路径是 `/Users/guohao/Documents/op7418的仓库/项目/Thin-Harness-Fat-Skills/ppt/index.html`。
...
| 图片页 | 图片是证据块,要么做 S22 主视觉,要么放进 S15 原始网格 | 不要使用未登记图文结构 |
...
| `t-meta` / `t-cat` local label | 正文页的 head 区可省略；需要时用于正文分点 / 图表 / 阶段。仅章节分割页放在大标题上方 |
| 大标题 | 章节/论点页必选;列表型小卡页可以用较小标题,但不能缺页级信息锚点 |
| `lead` 说明 | 可选;如果标题已经解释清楚,可以省略,但不能用长段正文贴着标题 |
| 图片 caption | S15 多图格必选;S22 大图可选,因为图已经是主视觉且下方有 KPI/说明 |
| 发丝线 / border-bottom | 可选;只能用于建立层级,不能为了装饰堆线 |
| KPI / 数字 | 只在有真实数据时使用;不要为概念解释编造数值 |
| `footnote` / 底部说明 | 可选;如果使用,必须避开 nav 安全区 |
| `S08 + Tier0 Map Component` | 地点/路线/人物住所关系专用;右侧地图必须有点、连线、卡片和 `+` / `-` / `DRAG` 控制,详见 `tier0-map-component.md` |
...
| 条件通用 | S04, S05, S15, S16 | 分别受数量、图片数量或单任务边界约束 |
| 数据专用 | S02, S06, S07, S18, S20, S21, S22 | 必须有真实时间、数值、指标或案例数据 |
| 结构专用 | S14, S17 | 必须有闭环或层级/生态关系;不适合普通段落 |
...
**默认推荐:`TIER0-COVER-EDITORIAL`** ⭐
- `<section class="slide light" data-layout="TIER0-COVER-EDITORIAL">`
- 白底顶部 mono chrome（左 deck 类型、右页码）+ 左侧 section label / IBM Plex 400 大标题 + 右侧短说明 + 底部发丝线。
- 默认不放 Logo、ASCII、工业节点、圆形或绿色角线；封面靠排版和留白建立识别。
- HTML / PPTX 必须由同一 `deck.json` 生成，标题换行和右侧短说明保持一致。
- **不要**再放编号大字「01」——页脚或 chrome 标页码即可

**关键类**:`.canvas-card` `.chrome-min` `.section-label` `.headline` `.rule`
**动效 recipe**:`hero`

**示例代码** — 见 `components-tier0.md` § TIER0-COVER-EDITORIAL

**可选变体**:`cover-dark` 只在用户明确要求深色封面时使用；不是默认。
...
**骨架**:2×3 网格 / 每格上方 IBM Carbon 图标 + 编号 + 短标题 + 一行描述 / 单元间用 hairline 分隔。
**关键类**:`.cell-6` `.cell-icon-row` `.cell-num`
**动效 recipe**:`six-cells` — 6 格按 z 形顺序点亮(L→R, T→B,每格延迟 90ms)
**注意**:**不要自己画 SVG 图标**,使用内联 IBM Carbon SVG（具体 path 见官方组件库）。
**示例代码**:
```html
<div class="cell-6">
  <div class="cell">
    <svg class="carbon-icon" viewBox="0 0 32 32" fill="currentColor" aria-hidden="true"><!-- verified Carbon path --></svg>
...
**骨架**:4 列均分,每列底部一根不同高度的 #B2ED1D 信号绿矩形(数据决定高度)+ 顶部图标 + 中段巨数 + 底部标签。
...
**用途**:整套 deck 收尾页（每 deck 一页）。

**默认推荐:`TIER0-CLOSING-SPLIT`** ⭐
- `data-layout="TIER0-CLOSING-SPLIT"`
- 左半 `#050B14` + ASCII 宣言；右半白底 3 条 takeaway
- 第 03 条：浅绿底 + 黑字 / `.step-num.is-active`；白底禁止绿字
- 依据 intake 的封底 CTA 决定：需要官网 CTA 时写 `data-closing-cta="tier0-app"`，加入 `data-website-qr="tier0-app"` + 可见网址 `www.tier0.app`。默认放在深墨左半页脚，使用白底深墨码；浅底变体改为深墨底白码，四周至少 12px 静区。关闭时写 `data-closing-cta="none"`，完全移除 QR 与网址。

**关键类**:`.slide.split` `.half.b-accent` `.ascii-bg`
**动效 recipe**:`split-statement`

**示例** — 见 `components-tier0.md` § TIER0-CLOSING-SPLIT
...
**骨架**:顶部大标题 / 中段可选一根 1px hairline 横线 + N 个节点。简单线性步骤用均布 node；每步需要一句解释时用交错的 `.process-weave`（黑色竖向 time strip、白色 stage、虚线 connector）。
**关键类**:`.timeline-h` `.tl-h-node` `.tl-h-axis` / `.process-weave` `.process-weave__stage`
**动效 recipe**:`timeline-walk` — 节点或阶段沿阅读顺序由左至右点亮。
...
**骨架**:上半屏左侧 t-cat + 大字 4 行宣言 + 右侧短段说明 / 下半屏 ink 通栏(无左右下边距)+ 反白短句 + IBM Carbon 图标矩阵。
...
### P13 · Site to Cloud Architecture · 现场到云端架构图

**用途**:展示两侧系统边界（例如现场 / 云端、数据源 / 应用层）如何通过 Tier0 核心连接。
**适用内容类型**:**两端系统边界 + 一个明确的核心连接层**。适用于本地设备、数据源、云服务、应用和模型之间的关系；不是三项概念卡，也不是点对点系统网。
**骨架**:顶部左对齐标题 + 右短说明 / 下方两个等宽 architecture board / 中间一条品牌绿单向桥接。每个 board 只有一个 `.architecture-node--core`，其余为 4–6 个可替换系统节点。
**关键类**:`.architecture-template` `.architecture-zone` `.architecture-board` `.architecture-node` `.architecture-bridge`
**动效 recipe**:`architecture-flow` — 标题进入 → 两个系统边界显现 → Tier0 core 与桥接箭头最后对齐。
**注意**:标签必须是 HTML，连线仅表达关系；不画多色箭头、阴影、复杂图标或交叉网线。品牌绿只给核心、活动节点和桥接。
...
### P15 · Image Evidence Grid · 2–4 图证据组

**用途**:用 2–4 张相关图片讲一件事，例如产品状态、现场流程、案例前后或界面细节。
**适用内容类型**:**一张主图 + 1–3 张辅助图**。每张图都要承担「主时刻 / 背景 / 细节 / 结果」中的一个角色；没有真实图片时保留明确槽位，不用假 Dashboard 或装饰库存图。
**骨架**:顶部左标题 + 右短说明 / 中段 12 列图片证据网格。4 图时为「左侧主图跨两行 + 右上宽图 + 右下两张细节」；`data-image-count="2|3|4"` 自动切换构图。
**关键类**:`.image-evidence-grid` `.image-evidence-item` `.image-evidence-slot` `.image-evidence-caption`
**动效 recipe**:`matrix-fill` — 先标题，再主图，最后按阅读顺序出现辅助图；不播放图片内部动效。
**注意**:同组必须统一图片比例与 `object-fit:cover`；caption 必填；不混用圆角、阴影或不同图片风格。

---

### P16 · Demo Evidence Player · 视频 / Live Web

**用途**:把一条产品任务作为可播放、可讲解的证据呈现。
**适用内容类型**:**单个受控用户任务**（录屏、live web、关键 UI 状态）；例如「从提示词创建交接班 App 并发布」。不是能力罗列，也不是产品设计稿或完整后台改版。
**骨架**:左 `4/12`（标签、观看任务、两条讲解要点）/ 右 `8/12`（16:9 直角播放器，带极简控制条）。
**关键类**:`.demo-player` `.demo-player__stage` `.demo-player__web` `.demo-player__controls`
**动效 recipe**:`demo-stage` — 左侧观看任务先到 → 播放器外框与证据内容入场；媒体不自动播放。
**注意**:播放器只承载**一条路径 + 一个最终状态**；实拍视频用 `<video controls muted playsinline>`，iframe 必须 sandbox；禁止圆角播放器、假 Dashboard、自动声音与多流程并列。
...
### P18 · Business Value · 三条论证 + 结果带

**用途**:用 3 条业务价值论证把「为什么现在值得做」讲清楚。
**适用内容类型**:**3 个业务结果 + 每项 2 条可验证支撑 + 一个可读的结果数字/短语**。每列只讲一条价值，不塞长段落或多个 KPI。
**骨架**:标题区高置 / 中段三等宽论证列（编号、IBM Carbon 图标、标题、摘要、两条 proof）/ 列底统一 outcome 带。默认是 30% 品牌绿；需要减轻“卡片感”时，改用共享顶线 + 竖向 `.argument-rail`，仅一列作为信号面。
**关键类**:`.s18-argument-grid` `.s18-value-card` `.s18-points` `.s18-metric`
**动效 recipe**:`why-now` — 标题先入 → 三列按阅读顺序出现 → outcome 结果带最后定场。
**注意**:三列底部 outcome 必须在同一基线；默认淡绿色为 30% 品牌色，hover 才变亮绿；不用粗体巨数或永久 active 卡。
...
**骨架**:大字标题 / 下方 4 列均分模块（编号 + 大字标题 + 段落 + 一张极简线条图）。交替使用灰面与 18% 浅绿面，卡片间距保持 20px；不再使用装饰性顶绿条。
**关键类**:`.four-cards` `.fc-col` `.line-sketch`
**动效 recipe**:`four-cards` — 标题先入 → 4 列从下向上推入(每列 110ms 错开)
...
**用途**:4–6 行核心数据的账单式展示。
**适用内容类型**:**4–6 项真实 KPI**，每行都有数值、可读标签、简短口径和一个状态点。适合财务、采用率、效率或交付数据；不替代柱状图比较。
**骨架**:全高等分 ledger；每行一道 hairline / 左侧同宽数值列 / 中部标签 / 右侧 mono 口径 / 最右状态点。最后一行可做单一的淡绿 focus。
**关键类**:`.ledger` `.ledger-row` `.ledger-num` `.ledger-label` `.ledger-context` `.ledger-icon`
**动效 recipe**:`stacked-ledger` — 标题先入；每行数字升起、标签左滑、状态点最后出现（180ms 错开）。
**注意**:用等高行填满内容区，不留无意义底部空洞；所有数值左沿、标签左沿、状态点右沿严格对齐。

---

### P21 · Tech Spec Sheet · 规格说明书

**用途**:选择正确的数据阅读方式，而不是把所有内容都塞进 KPI 或柱状图。
**适用内容类型**:**有真实来源的数据**。根据问题选择 `rank`（优先级）、`slope`（变化）、`waterfall`（贡献）或 `waffle`（构成）；每张图只回答一个问题。
**骨架**:左标题 + 右侧简短读法说明 / 下方 2×2 data-pattern field。每格放图型名称、阅读问题、最小图形；实际交付时补齐数值、单位、时间范围和来源。
**关键类**:`.data-pattern-grid` `.data-pattern` `.viz-mini` `data-viz="rank|slope|waterfall|waffle"`
**动效 recipe**:`tech-spec` — 标题先入，四种图形按阅读顺序轻推入。
**注意**:绿色只能标记焦点值或活动状态；没有来源时标为 `DATA PENDING`，不能伪造性能结论。
...
下面的 P23/P24 是早期为了探索图文混排加入的实验版式。它们不属于原始 22P,默认不要用于正式生成。除非用户明确说“我要实验新图文版式”,否则请使用 S22 或 S15 的图片槽位。

### P23 · P23 图文分栏 · 左文右图 / 右文左图(实验,默认禁用)
...
### P24 · P24 证据网格 · 多图证据墙(实验,默认禁用)

**用途**:三张同类型图片/截图/图表并列,展示证据链或多案例对比。
**适用内容类型**:**2-3 张同类图片**。适合 UI 截图重绘、流程图三段、三个案例实拍、三张数据小图。不同类型混放会破坏Tier0秩序。
...
| 现场 / 云端双系统架构 | P13 Site to Cloud Architecture |
| 闭环流程 / 自学循环 | P14 Loop Diagram |
| 2–4 张相关图片 / 截图证据 | P15 Image Evidence Grid |
| 单个产品任务录屏 / Live Web | P16 Demo Evidence Player |
...
| 地点 / 路线 / 人物住所关系 | S08 + Tier0 Map Component |
| 单图解释论点 / 图文混排 | P23 P23 图文分栏 |
| 2-3 张图片/截图/图表证据链 | P24 P24 证据网格 |
...
| 无数据,纯定性论断 | P3 / P10 Statement / P12 / P19 | ⚠️ **P7 H-Bar / P6 KPI Tower**(编造数据会被识破) |
| 4 项对等 | P19 Four Cards / P6(若有数据) | 不能强凑成 6 用 P4 |
| 6 项对等 | P4 Six Cells | 不能强凑成 4 用 P19 |
| 3 项对等 | P5 Sub-cards | |
| Before/After | P8 Duo Compare(必须正好 2 项) | |
| 地点/路线/城市关系 | S08 + Tier0 Map Component | 普通 S04 卡片罗列 |
...
| 2–4 张相关图片 | P15 Image Evidence Grid | 不要做成装饰性拼贴 |
| deck 收尾 | P9 Closing(每 deck 仅 1 次) | |
| 1 张核心图片 + 一段解释 | P23 P23 图文分栏 | P22(除非图片是主角且有 KPI) |
| 2-3 张同类图片 | P15 Image Evidence Grid | P24 为实验版，不用于正式 deck |
...
3. ❌ 自己画 SVG 图标 → ✅ 用内联 IBM Carbon SVG,保持图标来源一致
