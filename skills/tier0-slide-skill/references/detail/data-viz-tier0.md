...
## 5. 表格与「头条指标」布局（与第一性原理衔接）

有真实数据时，优先用视觉结构承载，而不是段落：

| 形态 | 组件 | 何时 |
|------|------|------|
| 账单式 KPI 行 | `.ledger` | 3–5 个并列大数 |
| 2×2 头条 | `.metric-quad` | 复盘四指标 |
| 底部指标轨 | `.kpi-rail` | 摘要页 / 封面结论加固 |
| 横线表 | `.data-table` | 实际 vs 目标、多列对比 |
| 比较条 | `.viz-bars` | 谁更快 / 谁更低 |

布局、疏密与「先写 coreClaim」见 `layout-first-principles-tier0.md`。禁止虚构数字；可把分散在源材料中的已有数字收成一行对比或小表。
