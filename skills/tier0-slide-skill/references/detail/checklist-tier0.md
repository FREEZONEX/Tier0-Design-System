...
Tier0 Slide Skill 专用清单。P0 项必须全部通过。完整 VI 见 `tier0-vi-style-guide.md`。

---

## 🔴 P0 · Tier0 专属（优先于下方版式骨架条目）
...
- 括号关键词 **500**；括号符号 **400**
- Step title: Plex Mono **500 only**，禁止 Bold 700
- 禁止 headline weight ≥600

### 0-T-3. 色彩与卡片底色

- accent 亮绿 `#B2ED1D` **仅**用于：marker、step-num active、signal-pill、6px 顶条、nav dot、**整块卡片底色（配黑字）**
- **禁止**在 `--paper` 白底上写 `color:var(--accent)` 或 `#B2ED1D` 文字 —— 对比度不足，看不清
- 白底上的「强调」用：**黑字** + 绿 marker / 绿底 pill / 浅绿卡片底，不要用绿字
- 大面积卡片底色优先：`--surface-ink` / `--surface-grey` / `--surface-green` / `--surface-grey-soft`
- 不要整页只用 `#F9F9F9` 极浅灰 —— 画面会发飘；每页至少 1–2 张 `--surface-grey` 或 `--surface-green` / `--surface-ink` 卡
- 禁止 accent `#002FA7` 满屏封面

### 0-T-3b. 绿字 vs 绿底（P0 · 对比度）

**现象**：section 小标题、`col-tag`、`t-meta` 用了 `color:var(--accent)`，在白底上几乎看不见。

**做法**：

| 场景 | ✅ 正确 | ❌ 错误 |
|---|---|---|
| 白底 label | `color:var(--ink)` 或 `var(--muted)` | `color:var(--accent)` |
| 需要绿色强调 | `.signal-pill` / `.col-tag.is-signal`（绿底黑字） | 绿字 |
| 白底 active 列标题 | 黑字 + `.signal-marker` | 绿字 |
| 时间线 active 节点 | 绿 dot + **黑字** step-title | `.th-node.accent .name { color:accent }` |
| 整块强调卡 | `.tier0-card--accent-solid` 或 `.card-accent-solid` | 浅绿底 + 绿字 |
| 亮绿满屏 `.slide.accent` | `color:var(--accent-on)` 黑字 | 旧模板白字 `#fff` / `rgba(255,255,255,...)` |

**模板已内置 override**：`.slide.light` 下 Tier0 遗留的 accent 文字色会被压回 `--ink`；`.slide.accent` / `.half.b-accent` 亮绿底同样用黑字，生成时仍不要主动写绿字或白字。
...
- 双格式默认封面：`cover-editorial`，白底左侧 Logo / 大标题 / 演讲者信息 + 右侧亮绿 ASCII 点阵；HTML 为动态点阵，PPTX 为可编辑静态点阵。
- HTML 与 PPTX 封面必须来自同一 `deck.json`，标题换行和右侧说明必须一致。
- `section-dark`：Gallery S10 纯深墨场，只保留章节标签、超大单句标题、可选短注释和底部发丝线；禁止硬网格及额外装饰。
- 封底: 固定 `TIER0-CLOSING-SPLIT`。以 `intake/request.json.closing.includeWebsiteAndQr` 为准：为 `true` 时使用 `data-closing-cta="tier0-app"`、官网二维码和可见网址 `www.tier0.app`；为 `false` 时使用 `data-closing-cta="none"`，不留二维码、网址或空 CTA 位。
- 需要二维码时：最小可视码区 `72×72px`，四周保留至少 `12px` 静区；深底使用白底深墨码，浅底使用深墨底白码。亮绿仅可做外围 marker，不得作为二维码本体。
- 需要二维码时：不可被页码、导航、页脚或动画遮挡；静态模式下仍可直接扫描。

### 0-T-6. 数据、Demo 与动效（新增）

- 数据图必须写清指标、单位、时间范围与来源；没有数据时标 `DATA PENDING`，不编 KPI。
- 默认黑/灰显示比较，亮绿 `#B2ED1D` 只突出一个 active / focus 数据点。
- 图表使用 `.viz-*` 组件或登记版式的原生图表，不用装饰性 SVG、渐变、3D 或彩虹色。
- Demo 使用真实 UI 截图优先；需要交互时，只实现一条可讲解的主路径，并置于 `.demo-evidence__stage`。
- 每页只能指定一个语义化 `data-animate` recipe；`B` 静态模式和 `prefers-reduced-motion` 下必须直接可读。

---

## 🔴 P0 · Tier0 骨架（继承 22 版式，Tier0 皮肤下仍适用）

### 0-L. 正文页必须来自原始 22P

**现象**:颜色、字体看起来像 Tier0,但标题跑到中间、图片不在网格上、页面结构和原始 22P 完全不是一套东西。
...
```bash
node <SKILL_ROOT>/scripts/validate-tier0-deck.mjs path/to/index.html
```
...
### 0-L-3. 地图页必须用 S08 Map Component

**现象**:地点/历史内容只画了简易 SVG 地图,没有真实点位、关系卡片、缩放/拖动控制,或滚轮触发了 PPT 翻页。

**做法**:
- 使用 `data-layout="S08"`。
- 先读 `references/tier0-map-component.md`。
...
### 0-L-4. 演示字号不能小到看不清 + 字重阶梯必须遵守

**现象**:Tier0页面整体结构没问题,但图注、说明、时间线、KPI note、卡片小字在投屏时看不清;或者 16px 小字用了 weight 300 导致又小又细。
...
**做法(字重阶梯 ⭐)**:
Tier0坚持"越大越细,越小越粗",字号与字重必须成反比阶梯:
...
- 封面/accent 反白大标题内强调字用 `italic + weight 300`,不要用 accent 色。
...
### 0-B. Tier0 head 区:kicker 必须在大标题"上方"(不要左右排)
...
### 0-B-2. Tier0 封面 / 转场 / 封底默认

**现象**:封面用满屏亮绿 + 白字看不清；或白底 + 重复「01」大编号。

**做法**(Tier0 必守):
- **封面默认** `TIER0-COVER-EDITORIAL` · 白底左侧 Logo / 大标题 / 演讲者信息 + 右侧亮绿动态 ASCII 点阵；PPTX 使用可编辑静态点阵降级
- **章节转场** `section-dark` · Gallery S10 纯深墨场 + 超大单句标题 + 底部发丝线；不得出现硬网格、Logo、绿色角线或角落圆形
- **封底默认** `TIER0-CLOSING-SPLIT` · 左 `#050B14` + ASCII / 右白底 3 条 takeaway；第 03 条可用浅绿底 + 黑字强调
- 可选：`slide.accent` 亮绿满屏页（模板示例）— 文字必须用 `--accent-on` 黑字，禁止白字
- 不要再写「01」等大编号：`.chrome-min` 已显示页码
- 封底/亮绿页 ASCII：`mix-blend-mode:screen` 由模板预设，勿随意改

**自检**:
- 封面 `data-layout="TIER0-COVER-EDITORIAL"`
- HTML 封面包含 `.cover-editorial__ascii > canvas.ascii-bg`，或双格式构建器生成的 `[data-ascii-field]`
- 双格式 JSON 首张 `kind:"cover-editorial"`
- `section-dark` 无 `logo` 字段、无 ellipse、无绿色 corner line
- 封底 `data-layout="TIER0-CLOSING-SPLIT"`
- 白底页无 `color:var(--accent)` 绿字

### 0-C. Tier0大字号双约束:`min(Xvw, Yvh)` 中 Y ≥ X × 1.6
...
### 0-D. Tier0图片混排:直角、同高、只做证据

**现象**:图片像普通 PPT 插图,圆角、阴影、比例混乱;多张截图高度不一,或 GPT-M 2.0 生成图自带标题/页脚,和页面 chrome 重复。

**根因**:Tier0的图片不是装饰,而是 grid 里的证据块。没有先选原始版式和图片槽位,就会把任意图片硬塞进页面。

**做法**:
- 先选版式:单张大图 + KPI 用 `S22`;2–4 图用 `S15` 的原始证据网格骨架改造；录屏 / Live Web 用 `S16` 播放器
...
- 目视:截图外侧背景应该是安静托底,不能比截图本身更抢眼;Tier0截图不得出现圆角和投影

### 0-D-2. Tier0底部分页安全区:最低处不要碰 nav
...
### 0-L-5. 模板还原度守卫:原始 PPT 是 golden source

**现象**:生成页看起来像Tier0,但和原始参考 PPT 的实际字重、间距、时间线、卡片密度不一致;越迭代越偏离参考。

**根因**:把新增图片版式或实验结构写成了全局样式修改,或无意改动了原始基座类,例如 `.h-hero` / `.h-xl` 字重、`.tl-node` 列宽、`.duo-compare` 间距。

**做法**:
- 原始参考文件 `/Users/guohao/Documents/op7418的仓库/项目/Thin-Harness-Fat-Skills/ppt/index.html` 是 Tier0 主题的 golden source,但要以**实际页面用法**为准,不要只看未使用的 CSS helper
...
- 新增图片能力必须绑定到 S22/S15 原始槽位；视频 / Live Web 必须绑定到 S16 `.demo-player`,不要发明新正文结构
- 如果要修改 `assets/template-tier0.html`,先做原始参考对比;可接受差异只应是 ASCII 类、S22 图片定位类、轻字重标题 helper 和已知动效修复

**自检命令**:
- 运行本次测试目录里的 `validate-tier0-deck.mjs`,确认输出里 `missing in template: 0`
...
- 通用版式(S03/S08/S11/S19)可多用;数据专用(S06/S07/S20/S21/S22)必须有真实数据或案例;结构专用(S14/S17)必须有闭环或层级关系;图片证据专用 S15 必须有 2–4 张相关真实图片

> **逐页验收门（必做）**：每一页完成后，按 `page-review-tier0.md` 检查信息接收、证据、层级、几何、安全区、留白和品牌；失败页面先修复再继续。全 deck 检查不能替代逐页检查。

### 0-G. 画布内容溢出时必须回调整体布局

**现象**: 复合版式页（如 S07 条形图 + 底部分点卡、S04 六格 + 长标题）在 `100vh` 的 `.canvas-card` 内纵向或横向溢出，底部被裁切、元素横向顶满甚至超出安全区。

**做法**:
- 生成后**必须在浏览器逐页检查** `.canvas-card` 是否装下全部内容；等入场动效结束后再判断，不要把动画中间态当成最终布局
- **优先删减次要内容**，而不是把字号/间距压到不可读：例如 S07 条形图从 5 条减到 4 条、分点卡从 4 条减到 3 条
- **同步收紧纵向节奏**：减小标题区 `margin-bottom`、图表 `gap`、底部组件 `padding-top`；仍溢出再略降标题字号
- **不要堆叠过多同层组件**：S07 推荐上限 ≈ **4 条 `.h-bar-row` + 3 条 `.point-card`**；S04 六格页不要再加底部长列表
- **复合页纵向节奏**：条形图与 `.point-card-list` 用 `margin-top:2~2.4vh` 衔接，**禁止**对分点列表用 `margin-top:auto`；分点列表底部留 `margin-bottom:8~9vh`，与 `#nav` / `#hint` 保持距离
- 横向溢出时检查是否误用了过宽 grid、inline 固定 `px` 宽度，或 chrome 安全区外的负 margin
- 改内容数量后，记得同步更新页内文案（如「5–10 items」与实际条数一致）
---
...
**做法**：
- **生成 PPT 前,必须先 `Read` 当前风格对应模板**:其他风格 读 `assets/template.html`,风格 B 读 `assets/template-tier0.html`,确认 layouts 里用到的类都已定义
...
**做法**：使用内联的 IBM Carbon SVG（从已安装的 Carbon icon 包或官方组件库复制 path），不依赖 CDN：

```html
<svg class="carbon-icon ico-md" viewBox="0 0 32 32" fill="currentColor" aria-hidden="true">
  <!-- Paste the verified IBM Carbon path here -->
</svg>
```

常用语义：target、layers、flow、security、launch、chart、network；具体路径见 IBM Carbon Icons。
...
- 7-8 页 Tier0 测试 deck 至少使用 6 个不同 S 编号版式
- 有 2-4 张配图时,使用 S15 图片证据网格；单张主视觉用 S22；视频证据单独使用 S16
...
其他风格 可以有轻微圆角。风格 B Tier0 必须直角: `.frame-img` 和图片本身都不要圆角、阴影或消费 app 式卡片感。
---
...
排版
  ...
  □ 逐页检查 `.canvas-card` 无纵向/横向溢出；复合页(S07 等)内容过多时优先删次要行/卡并收紧间距(见 0-G)
