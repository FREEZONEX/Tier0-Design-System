...
用于地理、历史、城市、人文路线、门店/校区/事件点位等内容。它不是新的正文版式,而是 **S08 Duo Compare 的右侧插槽扩展**:左侧仍是 Tier0 说明卡片,右侧替换为地图组件。

> 样式:点位/选中态使用 `#B2ED1D`；卡片用 `.tier0-card`。契约与 `tier0-map-component.md` 相同。
...
放到生成页 `<head>` 的额外 `<style>` 中,不要改 `template-tier0.html` 的全局基座类。
...
.wudadao-map,.tier0-map,.swiss-map{position:absolute;inset:0;background:#f4f4f0}
.wudadao-map.map-live .map-static,.tier0-map.map-live .map-static,.swiss-map.map-live .map-static{display:none}
  ...
  <div class="canvas-card">
    ...
    <div class="history-map-grid">
      ...
      <div class="map-panel">
        ...
        <div id="tier0-map" class="tier0-map" data-points='[填入 JSON]' data-relations='[填入 JSON]'>
...
放到 `</body>` 前。生成多张地图页时,把 id 从 `tier0-map` 改成唯一 id,并让初始化函数接收 selector。
...
(() => {
  ...
  function initTier0Map(){
    const el = document.getElementById('tier0-map');
  ...
  window.addEventListener('DOMContentLoaded', () => setTimeout(initTier0Map, 500));
})();
</script>
```
