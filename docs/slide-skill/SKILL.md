...
description: 将已有的 Tier0 PPT 结构、章节文档、图片、产品截图、Demo 视频、架构图或现有 PPTX，整理并逐页优化为符合 Tier0 品牌的演示稿。先理解演示目的、受众和素材证据，再选择或改造 Layout Gallery 的版式；在需要配图的槽位按 Tier0 illustration 风格统一出图（触发词 tier0_illustration_style）；可交付 HTML 演示稿、可编辑 PPTX 或两者。适用于「根据资料补全 Tier0 PPT」「优化现有 deck」「把 PPTX 改成 Tier0 风格」「按 Tier0 插画风格补概念配图」「用截图/视频做销售演示」「客户提案、售前、产品 Demo、架构说明、数据价值说明」等请求。本 skill 位于 Tier0 Design System 的 skills/tier0-slide-skill/；VI 冲突时服从仓库 tokens/deck.css 与 references/ppt-checklist.md；插画 DNA 与 tier0-illustration-style 对齐。
---
...
| `imageRole` | 用途 |
|-------------|------|
| `product-evidence` | 真实产品截图 / flat UI 证据 |
...
2. 真实截图 > 原生可编辑图 > Tier0 插画 > 纪实照片；不要用插画冒充产品证据。
...
1. 按 `imageRole` 放入真实素材、Tier0 插画或明确占位符；
2. 建立标题、辅助说明、证据 / 图形的阅读顺序；
3. 用 Tier0 token 和 Gallery 组件调整比例、间距、对齐与强调；
4. 仅在确有语义作用时添加 IBM Carbon 图标、连接线、色块或动效；
5. 抽象关系优先 Gallery 黑灰线条 + 单点亮绿；需要场景隐喻时用 §2b 的 `illustration`，不要改用其它插画口味。
